...
export const Button: React.FC<ButtonProps> = ({ 
  ...
  const variants = {
    primary: "bg-gradient-to-r from-neon-purple to-neon-blue text-white shadow-neon-blue/20 hover:shadow-neon-blue/40 hover:-translate-y-0.5",
    ...
    outline: "border-2 border-neon-blue/20 text-neon-blue bg-white hover:bg-neon-blue/5 hover:border-neon-blue dark:bg-slate-900 dark:border-slate-700 dark:text-neon-blue dark:hover:bg-slate-800",
...
export const Input: React.FC<InputProps> = ({ label, error, className = '', ...props }) => {
  return (
    <div className="flex flex-col gap-1.5 group">
      {label && <label className="text-sm font-semibold text-slate-600 dark:text-slate-400 group-focus-within:text-neon-blue dark:group-focus-within:text-neon-blue transition-colors uppercase tracking-widest text-[10px]">{label}</label>}
      <input
        className={`px-4 py-3.5 rounded-xl bg-slate-50 dark:bg-slate-800/50 border-2 ${error ? 'border-red-300 dark:border-red-500/50 focus:border-red-400 bg-red-50/30 dark:bg-red-900/10' : 'border-transparent focus:border-neon-blue/50 focus:shadow-neon-blue/10 focus:bg-white dark:focus:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-800'} outline-none transition-all duration-300 font-medium text-slate-800 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-slate-500 ${className}`}
        {...props}
      />
...
export const TextArea: React.FC<TextAreaProps> = ({ label, className = '', ...props }) => {
  return (
    <div className="flex flex-col gap-1.5 group">
      {label && <label className="text-sm font-semibold text-slate-600 dark:text-slate-400 group-focus-within:text-neon-blue dark:group-focus-within:text-neon-blue transition-colors uppercase tracking-widest text-[10px]">{label}</label>}
      <textarea
        className={`px-4 py-3.5 rounded-xl bg-slate-50 dark:bg-slate-800/50 border-2 border-transparent focus:border-neon-blue/50 focus:shadow-neon-blue/10 focus:bg-white dark:focus:bg-slate-800 outline-none transition-all duration-300 min-h-[100px] font-medium text-slate-800 dark:text-slate-100 hover:bg-slate-100 dark:hover:bg-slate-800 ${className}`}
        {...props}
      />
    </div>
  );
...
export const Header: React.FC<{ 
  ...
  <header className="bg-white/80 dark:bg-slate-950/80 backdrop-blur-md border-b border-slate-100 dark:border-slate-800/50 sticky top-0 z-20 px-5 py-4 flex items-center justify-between shadow-sm transition-all">
    <div className="flex items-center gap-3">
      ...
      <div className="flex items-center gap-3">
        {logo && <img src={logo} alt="Logo" className="w-10 h-10 object-contain rounded-lg shadow-neon-blue border border-neon-blue/30" />}
        <div>
          <h1 className="text-xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-neon-blue via-neon-purple to-neon-pink dark:from-neon-blue dark:to-neon-purple leading-tight drop-shadow-sm">{title}</h1>
